# Branch / Run Index — 2026-08-14 16:11 NZST
...
| V55 natural historical compounding | INCOMPLETE | `v55-natural-historical-compounding` | run `31764969750`, head `5863f4d92bfbd535205e49fbe12b1f29fd463a4d` | Only two READY BugsInPy worlds, both Black; no unique O1; natural compounding not reached. |
...
| IKKF V1 portable capability | ATTESTED WITH CORRECTION | `ikkf-v1-portable-capability` / related PR branch `agent/ikkf-portable-capability-v1` | primary run `31759600559` for strict V1 artifact; draft PR #1 contains related capstone packaging | Fresh compilation, second fresh compilation call, uninstall and reload are separately executed. Do not call C1/C2 stochastically independent: loss curves are bit-identical and the gate does not test weight/hash independence. |
...
| V77 SorryDB preflight | NEGATIVE AT CURRENT SOLVER/BUDGET | `main` | repaired run previously tracked as `31761636184` | Exact-commit plumbing/official verifier pass; all three repos prepare; five solver arms solve 0/3 tasks. |
...
- GitHub Actions `conclusion=success` means the workflow completed, not automatically that the scientific hypothesis passed.
