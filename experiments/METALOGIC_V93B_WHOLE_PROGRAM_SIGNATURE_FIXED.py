...
OUT=Path(os.environ.get('OUT_DIR','results/v93b')); OUT.mkdir(parents=True,exist_ok=True)
SEED='V93B_WHOLE_PROGRAM_SIGNATURE_FIXED_2026-08-14'
...
def permute_prototypes(prototypes):
    vocab=sorted({k for p in prototypes for k in p})
    if not vocab:return prototypes
    perm=sorted(vocab,key=lambda k:h('null|'+k))
    mapping={vocab[i]:perm[i] for i in range(len(vocab))}
    return [Counter({mapping[k]:v for k,v in p.items()}) for p in prototypes]
...
def main():
    ...
    null_prototypes=permute_prototypes(prototypes)
    ...
    for n in test:
        ...
        scored_null=sorted(((max((cosine(delta(bug,c),p) for p in null_prototypes),default=0),c) for c in cands),key=lambda x:-x[0])
        ls=ns=us=False
    ...
    new=sorted(set(learned)-set(null)); null_only=sorted(set(null)-set(learned))
    gates={'preexisting_external_corpus':True,'heldout_correct_files_sealed':True,'nonempty_training_signatures':bool(prototypes),'learned_beats_null':len(learned)>len(null),'unique_learned_gain':bool(new)}
    res={'protocol':'V93B_WHOLE_PROGRAM_SIGNATURE_FIXED','external_commit':COMMIT,'train':train,'test':test,'learned_solved':learned,'null_solved':null,'unrestricted_solved':unrestricted,'unique_learned_gain':new,'null_only_gain':null_only,'rows':rows,'gates':gates,'verdict':'PASS_WHOLE_PROGRAM_SIGNATURE_V93B' if all(gates.values()) else 'MIXED_WHOLE_PROGRAM_SIGNATURE_V93B','qualification':'Supervised bridge. Whole-program AST graph-delta signatures are induced from training-side human fixes; held-out correct implementations remain sealed. The matched null permutes signature coordinates while preserving vector sizes/values. No semantic operator/role labels are supplied. A PASS would show transfer of global change-shape priors, not autonomous ontology invention.'}
