...
SEED='V52F_HISTORICAL_METADATA_COVERAGE_20260814'; OUT=Path('artifacts/v52f'); OUT.mkdir(parents=True,exist_ok=True)
BIP=Path('/tmp/BugsInPy'); WORK=Path('/tmp/v52f_work'); WORK.mkdir(exist_ok=True)
MAX_EP=24; MAX_SITES=24; DESTS=sorted(s for s in tokenize.EXACT_TOKEN_TYPES if 1<=len(s)<=2); OLD=['IDENTITY','REVERSE_WINDOW','ROTATE_LEFT','ROTATE_RIGHT','SWAP_ADJACENT']
...
def checkout(project,bug):
 ...
 rc,o=run(cmd,t=90); repo=root/project
 if repo.exists():(repo/'.v52_test_files').write_text(info(BIP/'projects'/project/'bugs'/bug/'bug.info').get('test_file',''))
 return rc,o,repo
def decode_req(p):
 b=p.read_bytes()
 if b'\x00' in b:
...
def setup(repo,py):
 ...
 pip=repo/'.v52env/bin/pip'; run(f'{pip} install -q "setuptools<70" "packaging<24" "pytest<8" coverage',repo,60)
 ...
 if req.exists():
  ...
  for s in decode_req(req).splitlines():
   ...
   if sys.platform!='win32' and re.match(r'(?i)^pywin32(?:[<=>].*)?$',s):continue
   lines.append(s)
  (repo/'.v52req.txt').write_text('\n'.join(lines))
  if lines:
   rc,x=run(f'{pip} install -q --use-deprecated=legacy-resolver -r .v52req.txt',repo,120);chunks.append(x)
   if rc:return False,'\n'.join(chunks)
...
def test(repo):
 h=repo/'bugsinpy_run_test.sh'
 if not h.exists():return False,'MISSING_HARNESS'
 z=run('bash bugsinpy_run_test.sh',repo,45,env(repo));return z[0]==0,z[1]
def reset(repo):run("git reset --hard -q HEAD && git clean -fdxq -e .v52env -e 'bugsinpy_*' -e .v52req.txt -e .v52_test_files",repo,20)
def coverage_sites(repo):
 reset(repo); files=(repo/'.v52_test_files').read_text(errors='ignore').split(';') if (repo/'.v52_test_files').exists() else []
 files=[x.strip() for x in files if x.strip() and (repo/x.strip()).exists()]
 if not files:return []
 run('.v52env/bin/coverage erase',repo,10,env(repo));run('.v52env/bin/coverage run --parallel-mode -m pytest -q '+' '.join(repr(x) for x in files),repo,45,env(repo));run('.v52env/bin/coverage combine >/dev/null 2>&1 || true; .v52env/bin/coverage json -o .v52cov.json >/dev/null 2>&1 || true',repo,20,env(repo))
 p=repo/'.v52cov.json';sites=[]
 if not p.exists():return sites
...
for i,(pr,bg,py) in enumerate(S,1):
 ...
 if not good:ev['status']='INFRA_SETUP';ev['tail']=so[-300:];events.append(ev);continue
 base,_=test(repo);ev['baseline_pass']=base
 if base:ev['status']='NONREPRODUCING';events.append(ev);continue
 sites=coverage_sites(repo);ev['coverage_site_count']=len(sites)
 ...
 events.append(ev)
R={'protocol':SEED,'events':events,'learned_operator':op,'causal_reuse':reuse_ev};R['gates']={'historical_bug_reproduced':any(e.get('baseline_pass') is False for e in events),'operator_formed':op is not None,'old_closure_obstruction':any(e.get('old_closure_obstruction') for e in events),'different_project_reuse':bool(reuse_ev and op and reuse_ev['project']!=op['origin_project']),'ablation_restores_failure':bool(reuse_ev),'fix_withheld':True};R['verdict']='PASS_V52F_HISTORICAL_METADATA_COVERAGE' if all(R['gates'].values()) else 'INCOMPLETE_V52F_HISTORICAL_METADATA_COVERAGE';(OUT/'RESULT.json').write_text(json.dumps(R,indent=2));print(json.dumps(R,indent=2))
