...
ROOT=v100.v99.ROOT; full_score=v100.v99.full_score; base_candidates=v100.balanced_rich_candidates
...
def expr_candidates(src,cap):
    ...
    call_nodes=[n for n in ast.walk(t) if isinstance(n,ast.Call)]; ret_nodes=[n for n in ast.walk(t) if isinstance(n,ast.Return)]
    for typ,node,j,nm in sites:
        for op in (ast.Add(),ast.Sub()):
            ...
            if typ=='call':
                idx=call_nodes.index(node); zc=[n for n in ast.walk(z) if isinstance(n,ast.Call)]
                if idx<len(zc) and j<len(zc[idx].args): zc[idx].args[j]=rep; emit(z,'EXPR_GROW')
            else:
                idx=ret_nodes.index(node); zr=[n for n in ast.walk(z) if isinstance(n,ast.Return)]
...
def main():
    ...
    for n in test:
        ...
        bk=reachable(n,b); ek=reachable(n,b+e)
    ...
    res={'protocol':'V102_FRESH_SPLIT_EXPRESSION_CONSTRUCTOR','external_commit':v100.v99.COMMIT,'test':test,'excluded_exposed':sorted(EXPOSED),'base_reachable':base_s,'expanded_reachable':expr_s,'new_closure':new,'rows':rows,'gates':gates,'verdict':verdict,'qualification':'Fresh-split representation bridge only. The generic typed expression-construction substrate is supplied, not learned. Correct implementations are never read. A PASS supports expression construction as a missing constructor class, not autonomous constructor genesis.'}
