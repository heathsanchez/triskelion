...
def predname(pred):
    return ' & '.join(('+' if sign else '-')+relname(r) for r,sign in pred)
def applies(pred,rels): return all((r in rels)==sign for r,sign in pred)

# Frozen external sites. Their behavioral roles were established in V39/V40/V40b;
# V41 reruns the unchanged repository tests rather than trusting source equality.
...
# Candidate language is generated from relations actually observed at the sites:
# all one-literal predicates, then all two-literal conjunctions.
...
R={
 'protocol':'V41_GENERIC_AST_RELATION_SCOPE_CONSTRUCTION_20260814',
...
# Verify the old undifferentiated scope is insufficient.
...
click_preserved,click_log=ck_count()
reset(RH); replace(rich_path,'cursor < len(plain)','cursor <= len(plain)')
rich_preserved,rich_log=rh_suite()
R['old_scope']={
 'seeded_trigger_passes':old_trigger,
 'repair_restores_trigger':repair_trigger,
 'broad_rule_preserves_click':click_preserved,
 'broad_rule_preserves_rich':rich_preserved,
 'rich_log_tail':rich_log[-800:],
}

# Construct from behavior: cover both positives, exclude both protected negatives.
valid=[]
for p in cands:
    if applies(p,rels['trigger']) and applies(p,rels['transfer']) and not applies(p,rels['protected_click']) and not applies(p,rels['protected_rich']):
        valid.append(p)
...
R['construction']={
 'candidate_count':len(cands),'valid_count':len(valid),'minimal_count':len(minimal),
 'minimal':[predname(p) for p in minimal],'selected':predname(selected) if selected else None,
}
...
transfer_old,old_log=rq_slice()
if selected and applies(selected,rels['transfer']):
    replace(transfer_path,'slice_length < 0','slice_length <= 0')
...
# Later independently authored counterevidence arrives in the discovered relation.
reset(DJ); contra_base,_=dj_pass()
if selected and applies(selected,rels['counterexample']):
    replace(contra_path,'if len(password) < self.min_length:','if len(password) <= self.min_length:')
...
# Re-identify under accumulated evidence. If nothing remains, revoke.
consistent=[]
for p in cands:
    if (applies(p,rels['trigger']) and applies(p,rels['transfer']) and
        not applies(p,rels['protected_click']) and not applies(p,rels['protected_rich']) and
...
R['gates']={
 ...
 'excludes_two_structurally_distinct_protected_sites':selected is not None and not applies(selected,rels['protected_click']) and not applies(selected,rels['protected_rich']),
...
R['verdict']='PASS_V41_GENERIC_RELATIONAL_CONSTRUCTION' if all(R['gates'].values()) else 'FAIL_V41_GENERIC_RELATIONAL_CONSTRUCTION'
R['claim_boundary']='The decisive If.test distinction is not supplied as a feature or scope token. It is synthesized as an ancestor-node/field relation from a frozen generic AST relational primitive and external executable evidence. Python AST node/field ontology remains supplied; this does not claim ontology invention outside that substrate.'
