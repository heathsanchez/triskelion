...
P={"name":"METALOGIC_WAKE_SLEEP_STAIRCASE_V5","date":"2026-08-13","seed":20260813,"base":"Qwen/Qwen3.5-9B","rank":32,"lr":2e-4,"steps_per_stage":3,"threshold":0.75,"train_n":16,"heldout_n":8,"claim_boundary":"Synthetic cross-session test of lineage-specific successor acquisition and simultaneous retention under non-conflicting task contexts. No rehearsal of ancestor tasks after their stage; not external-task/open-ended evidence."}
OUT=Path('artifacts/wake_sleep_staircase_v5'); OUT.mkdir(parents=True,exist_ok=True)
...
# Generation A + matched wrong ancestor. Each capability has a distinct task key.
with client.session(project='ml-v5-A') as s:
 lin=mk(s); w1=mk(s); cA=train_curve(lin,'A',A,'A_lineage'); cW1=train_curve(w1,'W1',W1,'W1_wrong'); aacc=evaluate(lin,'A',A,'A_final'); ckA=lin.save_weights('A_training',mode='training'); ckW1=w1.save_weights('W1_training',mode='training'); R['stages']['A']={'curve':cA,'heldout':aacc,'checkpoint':ckA.path,'wrong_checkpoint':ckW1.path}; log('sleep_A',checkpoint=ckA.path)
# Generation AB: no A rehearsal. Correct A ancestry vs cold vs unrelated W1 ancestry.
with client.session(project='ml-v5-AB') as s:
 ...
 curves={'lineage':train_curve(lin,'AB',AB,'AB_lineage'),'cold':train_curve(cold,'AB',AB,'AB_cold'),'wrong':train_curve(wrong,'AB',AB,'AB_wrong')}
 retention_after_AB={'A':evaluate(lin,'A',A,'retention_A_after_AB'),'AB':evaluate(lin,'AB',AB,'retention_AB_after_AB')}
 _=train_curve(wrongchain,'W2',W2,'W2_wrong_chain'); ckAB=lin.save_weights('AB_training',mode='training'); ckW12=wrongchain.save_weights('W1W2_training',mode='training'); R['stages']['AB']={'pre':pre,'curves':curves,'retention':retention_after_AB,'checkpoint':ckAB.path,'wrong_chain_checkpoint':ckW12.path}; log('sleep_AB',checkpoint=ckAB.path)
# Generation ABC: no A/AB rehearsal. Test coexistence after learning C-context successor.
with client.session(project='ml-v5-ABC') as s:
 ...
 curves={'lineage':train_curve(lin,'ABC',ABC,'ABC_lineage'),'cold':train_curve(cold,'ABC',ABC,'ABC_cold'),'wrong':train_curve(wrong,'ABC',ABC,'ABC_wrong')}
 retention={'A':evaluate(lin,'A',A,'retention_A_final'),'AB':evaluate(lin,'AB',AB,'retention_AB_final'),'ABC':evaluate(lin,'ABC',ABC,'retention_ABC_final')}; ckABC=lin.save_weights('ABC_training',mode='training'); R['stages']['ABC']={'pre':pre,'curves':curves,'retention':retention,'checkpoint':ckABC.path}
def metrics(stage):
...
R['gates']={
 ...
 'G4_A_retained_after_AB':R['stages']['AB']['retention']['A']>=P['threshold'],
 'G5_ABC_lineage_faster_cold':better(MABC['lineage']['dose'],MABC['cold']['dose']),
 'G6_ABC_lineage_faster_wrong':better(MABC['lineage']['dose'],MABC['wrong']['dose']),
 'G7_A_retained_final':R['stages']['ABC']['retention']['A']>=P['threshold'],
 'G8_AB_retained_final':R['stages']['ABC']['retention']['AB']>=P['threshold'],
 'G9_ABC_acquired':R['stages']['ABC']['retention']['ABC']>=P['threshold']}
R['verdict']='PASS_WAKE_SLEEP_STAIRCASE_V5' if all(R['gates'].values()) else 'FAIL_OR_MIXED_WAKE_SLEEP_STAIRCASE_V5'; (OUT/'FINAL_RESULT.json').write_text(json.dumps(R,indent=2)); log('final',verdict=R['verdict'],gates=R['gates']); print(json.dumps(R,indent=2),flush=True)
